 ...
 */
@Module(name = "mqtt", schemaVersion = "1.0", friendlyName = "MQTT", minMuleVersion = "3.3.0", description = "MQTT Module")
public class MqttModule
{
    ...
    /**
     * File Persistence Location - directory on the machine where message persistence can be stored
     * to disk.
     */
    @Configurable
    @Optional
    ...
    /**
     * Convenience method that connects to a given MQTT broker. Returns the MqttClient object for
     * later use with publish and subscribe.
     * 
     * @param clientId
     * @return MqttClient
     * @throws ConnectionException
     */
    private MqttClient connectClient(final String clientId) throws ConnectionException
    {
        LOGGER.info("Creating client with ID of " + clientId);
        ...
        if (getPersistenceLocation() != null || StringUtils.isNotBlank(getPersistenceLocation()))
        {
            ...
            {
                LOGGER.error("Error creating file persistence for messages...persistence will be IGNORED!!!",
                    e);
            }
        }
        ...
        if (StringUtils.isNotBlank(this.getLwtTopicName()))
        {
            ...
            final MqttTopic lwtTopic = this.client.getTopic(this.getLwtTopicName());
    ...
    public byte[] publish(final String topicName,
                          ...
                          @Optional @Default("1") int qos,
                          @Payload final Object messagePayload,
        ...
        throws MqttException, IOException
    {
        ...
        {
            this.client.connect();
            LOGGER.info("Connected to " + getBrokerHostName() + " on port " + getBrokerPort() + " as "
                        + getClientId());
        }
        else
        {
        ...
        final MqttTopic topic = this.client.getTopic(topicName);

        LOGGER.debug("Preparing message...");
        final MqttMessage mqttMessage = new MqttMessage(payloadBytes);
